...
def pos_tag(test_words, transition_probability, emission_probability):
    ...
    for sentence in test_words:
        ...
        for tag in dp[word_counter]:
            ...
            cur_probability = Decimal(
                Decimal(prior_probability)
                * Decimal(transition_probability["<end>"][tag1])
            )
            if cur_probability > max_probability:
                max_probability = cur_probability
                max_tag = tag
...
def run():
    ...
    with open("data/pos_tagger_probability.json", "r", encoding="utf8") as f:
    ...
    test_tags = pos_tag(words, transition_probability, emission_probability)
