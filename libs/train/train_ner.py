...
def read_train_file():
    ...
    for annotations in dataset.split("\n\n"):
        ...
        for line in annotations.split("\n"):
            ...
            word_count = dict_increase(word_count, w, 1)
        words.append(lw)
        tags.append(lt)
        lw = []
        lt = []
...
def train(train_list_words, train_list_tags, tag_count, word_count):
    ...
    for tags in train_list_tags:
        ...
        transition_probability["O"] = dict_increase(
            transition_probability["O"], prev_tag, 1
        )
    ...
    for tag1 in tag_count:
        ...
        for tag2 in tag_count:
            if tag1 not in transition_probability[tag2]:
                transition_probability[tag2][tag1] = (
...
def run():
    ...
    with open("data/ner_probability.json", "w") as f:
